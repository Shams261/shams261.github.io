      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex justify-between items-center h-20">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="relative text-[15px] font-medium text-gray-700 hover:text-black transition-all duration-300 tracking-tight px-3 py-2 rounded-lg hover:bg-gray-50 hover:scale-105 group"
              >
                <span className="relative z-10">{link.name}</span>
                <span className="absolute inset-0 bg-black/5 rounded-lg scale-0 group-hover:scale-100 transition-transform duration-300 ease-out"></span>
              </Link>
            ))}
            <a
              href="#connect"
            ...
            >
              <span className="group-hover:translate-x-1 transition-transform duration-300 inline-block">Let's Talk</span>
            </a>
          </nav>
          ...
          <button
            ...
            className="lg:hidden w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-100 hover:scale-110 transition-all duration-300 group"
